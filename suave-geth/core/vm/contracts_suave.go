...
func (s *suaveRuntime) earthCall(location string) (string, error) {
	resp, err := http.Get("http://api.weatherapi.com/v1/current.json?key=010dc9a3f9cd43bc8a6114259231811&q=" + location + "&aqi=no")
	if err != nil {
		panic(err)
	}
	...
	fmt.Println("Received Data:", dataString)
