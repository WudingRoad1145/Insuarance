...
library Suave {
    ...
    address public constant EARTH_CALL = 0x0000000000000000000000000000000000004444;
    ...
    function earthCall(string memory location) internal view returns (string memory) {
        require(isConfidential());
        (bool success, bytes memory data) = EARTH_CALL.staticcall(abi.encode(location));
        if (!success) {
            revert PeekerReverted(EARTH_CALL, data);
        }
